...
def main():
    ...
    if len(sys.argv)<3:
        print("Invalid # of arguments. Usage: AutoTester.py #cases exe_path",file=sys.stderr)
        return  
    try:
    ...
    for i in range(N_TESTCASES):
        ...
        num_seq = list( map(str,(random.randint(1,N_MAXINT) for _ in range(num_nums))) )
    ...
    for i,test in enumerate(test_cases):
        ...
        if error==0:
            ...
            print("FAIL! Extreme error of",error,"detected")#,file=sys.stderr)
            wrong += 1
        print("-----------------------------------------")
    print("Finished. {}/{} Correct ({} wrong). {:.2f}% Failure Rate".format(N_TESTCASES-wrong,N_TESTCASES,wrong,wrong/N_TESTCASES*100))

def getOutput(testcases,fname):
    proc = Popen(fname,stdin=PIPE,stdout=PIPE,stderr=PIPE,universal_newlines=True)
    proc.stdin.write("\n".join(testcases)+"\n#")
    proc.stdin.close()
    #take only the non-empty lines and remove the trailing newline
    output = [line.rstrip() for line in proc.stdout.readlines() if len(line)>1]
    proc.kill()
